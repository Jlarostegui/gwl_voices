...
export class WgService {
  ...
  getAllWorkingGroups(): Promise<any[]> {
    let result = lastValueFrom(this.httpClient.get<any[]>(ROOT_URL))
    return result
  };

  // OK => Devuelve un Object
  getWorkingGroupById(pId: number): Promise<Working_groups> {
    let response = lastValueFrom(this.httpClient.get<any>(`${ROOT_URL}/${pId}`));
    return response
  }
  ...
  addWorkingNewWorkingGroup(pWg: Working_groups): Promise<Working_groups> {
    ...
    let response = lastValueFrom(this.httpClient.post<Working_groups>(`${ROOT_URL}`, pWg, httpOptions));
  ...
  updateWorkingNewWorkingGroup(pWg: Working_groups): Promise<Working_groups> {
    ...
    let response = lastValueFrom(this.httpClient.put<Working_groups>(`${ROOT_URL}`, pWg));
    console.log(response)
    return response
  }
  ...
  deleteWorkingGroup(pId: number): Promise<any> {
    let response = lastValueFrom(this.httpClient.delete<any>(`${ROOT_URL}/${pId}`));
    console.log(response)
    return response
  }
}
